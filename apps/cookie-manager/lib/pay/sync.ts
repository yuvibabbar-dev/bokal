...
import { shouldContactBilling } from './engagement';
...
export async function syncEntitlementCache(): Promise<EntitlementCache | null> {
  // Privacy gate: free users who never engaged Pro never contact the billing server.
  if (!(await shouldContactBilling())) return null;
  try {
