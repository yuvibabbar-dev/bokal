# Bokal — Milestone 4: Pro Layer — Plan
...
**Interfaces:** `interface EntitlementCache { paid: boolean; checkedAt: number }`; `const CACHE_KEY = 'bokal:entitlement'`; `function isEntitled(cache: EntitlementCache | null, now: number, graceMs: number): boolean`; config `EXTPAY_APP_ID`, `GRACE_MS`, `USE_MOCK_BILLING`.
...
export const EXTPAY_APP_ID = 'bokal-cookie-manager';
...
```ts
  chrome.alarms.create('bokal:entitlement', { periodInMinutes: 60 * 24 });
  chrome.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === 'bokal:entitlement') void import('../lib/pay/sync').then((m) => m.syncEntitlementCache());
  });
```
...
```md
# Bokal Pro — activating ExtPay (currently mock mode)

Pro entitlement runs through the `Billing` interface (`lib/pay/billing.ts`). Today `getBilling()`
returns `MockBilling` (entitlement via the `bokal:mockPaid` local flag; `openUpgrade()` sets it).
