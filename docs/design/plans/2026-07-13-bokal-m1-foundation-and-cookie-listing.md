...
> `@types/react`/`@types/react-dom` are required for `tsc` to type-check JSX (WXT's Vite build transforms JSX without them, but `tsc --noEmit` needs them).

- [ ] **Step 2: Create tsconfig + WXT config**

`apps/cookie-manager/tsconfig.json` — extend WXT's generated tsconfig (auto-imports, `@wxt-dev/module-react`, strict, path aliases) and layer on our extra flags. Do **not** set a restrictive `types` array (it would exclude React/other `@types`), and do not extend the plain shared base (it misses WXT globals like `defineBackground`):
```json
{
...
```
> `.wxt/tsconfig.json` is generated by `wxt prepare` (runs on `postinstall`), so it always exists after `pnpm install`. `jsx` is needed for `tsc --noEmit` type-checking even though Vite handles the actual JSX transform.
