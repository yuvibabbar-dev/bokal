...
- **Blocking is reactive**: a blocked cookie is set by the site, then removed by Bokal — not prevented at the network layer. Every piece of UI copy must say so (honesty invariant).
- Protected cookies must never be deleted by any Bokal action (single delete, delete-all, or a future profile apply).
...
- Produces:
  ...
  - `loadRules(): Promise<Rules>` (storage.local key `bokal:rules`, defaults to empty arrays)
...
- [ ] **Step 3: Implement `rules.ts`** — `RULES_KEY = 'bokal:rules'`; `loadRules` reads + fills defaults; `matchesBlock` strips a leading dot from both sides and checks `domain === b || domain.endsWith('.' + b)`; `sortPinned` returns `[...pinned in original order, ...rest in original order]`.
...
- [ ] **Step 3:** `rules-store.ts` — vanilla zustand store loading rules on init + `chrome.storage.onChanged` sync (key `bokal:rules`); actions mutate via `toggleId`/array ops + `saveRules` + local set. Mirror `entitlement-store` structure.
...
- [ ] **Step 2:** `BlockRules.tsx` — input + "Block domain" button (adds via `rulesStore.addBlock`), list of blocked domains each with a remove ✕. Header copy: "Blocked domains — Bokal deletes matching cookies as soon as they're set (reactive, not network-level)." No cookie values shown.
...
- [ ] **Step 3:** `devtools/main.ts` — `chrome.devtools.panels.create('Bokal', '', 'devtools-panel.html', () => {});`. `devtools/index.html` loads it.
- [ ] **Step 4:** `devtools-panel/main.tsx` — `import '@bokal/ui-kit/theme.css'`; call `setInspectedTab(chrome.devtools.inspectedWindow.tabId)` before `createRoot(...).render(<App />)`. `devtools-panel/index.html` mirrors the sidepanel html.
