# Wafer Pro — ExtPay billing (WIRED, M12)
...
- `lib/pay/config.ts`: `EXTPAY_APP_ID = 'wafer-test'` (the sandbox app — **swap to the production id
...
- **Free users never contact ExtPay.** `syncEntitlementCache` is gated by `shouldContactBilling()`
  ...
  (`wafer:proEngaged`) or holds a paid cache within grace. `startBackground()` only registers a
...
- **No `onPaid` content script.** Using ExtPay's `onPaid`/`onTrialStarted` would require a
  ...
  content script (and any new host access), Wafer instead re-checks entitlement on panel
...
1. Register the extension id `wafer-test` on extensionpay.com (or change the id in config to your
...
2. Build + load unpacked (`pnpm --filter @wafer/cookie-manager build`, load `.output/chrome-mv3`).
3. Grant Wafer access to a site, open the panel, click **★ Unlock Pro** → ExtPay page (test mode
