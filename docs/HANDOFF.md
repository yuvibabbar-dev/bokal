...
1. ~~Wire real ExtPay~~ → **WIRED (M12), sandbox test RUN 2026-07-13 — one wiring issue found, awaiting the founder's new app id.**
   ...
   **Sandbox result:** the founder paid with 4242… successfully (ExtPay billing portal shows an active "Bokal-test" $4.00/mo subscription, invoice Paid) — BUT the portal is branded **"Couples Companion"**: the id `bokal-test` resolves to the founder's *Couples Companion* ExtPay app, so the payment landed in THAT app's connected Stripe, not the Bokal Stripe (why his Bokal Stripe dashboard shows nothing). Not a code bug.
