# Bokal — Store Submission Guide

> Ordered checklist to submit Bokal to the Chrome Web Store and Microsoft Edge Add-ons.
...
- `zip` runs `wxt zip`, which rebuilds and packages the same output into
  `apps/cookie-manager/.output/bokalcookie-manager-<version>-chrome.zip` (e.g.
  `bokalcookie-manager-1.0.0-chrome.zip`, ~83 KB). This is the artifact to upload to the Chrome
...
- Do **not** set `BOKAL_E2E=1` when building the artifact for submission — that env var (used
...
- Before uploading, spot-check `apps/cookie-manager/.output/chrome-mv3/manifest.json`: no
  ...
  **⚠ Run a plain `pnpm --filter @bokal/cookie-manager build` first.** If you last ran `build:e2e`
...
`wxt zip -b firefox`, but Bokal does not target Firefox yet. Firefox's MV3 implementation uses
