# Wafer — Post-M7 Feature Roadmap (evidence-based, payments excluded)

**Date:** 2026-07-13 · **Method:** deep-research harness — 5 search angles, 23 sources fetched, 113 claims extracted, top 25 adversarially verified (3 independent votes each): **20 confirmed, 5 refuted**. Effort sizes (S/M/L) are grounded in the actual Wafer codebase (post-M7, `master`), not generic guesses.
...
- **Multi-account session switching** (SessionBox class — the feature nearest Wafer's Pro profiles): no claims about SessionBox/J2TEAM or session-switching demand survived verification. Research CWS review text specifically before building toward it.
